...
<task type="auto">
  ...
  <action>
    ...
    **Toolbar redesign (src/toolbar/Toolbar.tsx):**
    ...
    - Remove: permissions-only toggle button and its `handlePermissionsToggle` callback (CTRL-04 killed per CONTEXT.md -- subgraph + row dimming replaces this).
...
<task type="auto">
  ...
  <action>
    ...
    **Clean up store references (legend):**
    ...
    **Remove permissionsOnly filter (CTRL-04 killed per CONTEXT.md):**
    - In `src/types.ts`: remove `permissionsOnly: boolean` from `GraphFilters` interface.
    - In `src/store/viewer-store.ts`: remove `permissionsOnly: false` from `DEFAULT_FILTERS`.
    - In `src/graph/traversal.ts`: remove the `noPermissionFilter` variable and simplify the early-return guard in `applyFilters()` that checks it. The `noTypeFilter` check alone remains — if `types` is empty, return all nodes/edges unchanged. Remove any filter logic that was gated on `permissionsOnly`.
    - In `src/toolbar/Toolbar.tsx`: the permissions-only button was already removed in Task 1. Remove the `handlePermissionsToggle` callback and any remaining `filters.permissionsOnly` references if not already cleaned up.

    **Verify no dangling imports:**
    - Search for any remaining imports of `LegendPanel`, `legendOpen`, `toggleLegend`, or `permissionsOnly` across the codebase.
    - Fix or remove any found references.

    **Note:** The Toolbar.tsx legend button and permissions-only button were already removed in Task 1. If any references remain in App.tsx from Task 1, clean those up here.
  </action>
  <verify>
    Run `npm run build` -- must compile without errors.
    Run `npm run lint` -- must pass.
    Grep for "LegendPanel", "legendOpen", "toggleLegend", "permissionsOnly" in src/ -- zero results.
  </verify>
  <done>
    LegendPanel component deleted. All references to legendOpen, toggleLegend, and permissionsOnly removed from store, types, traversal, and components. No dangling imports. Clean build and lint.
  </done>
</task>
