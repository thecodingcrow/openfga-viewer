  ...
  <action>
    ...
    **Component structure:**
    ...
    - Subscribe to relevant store state:
      ...
      - `s.nodes` and `s.edges` from viewer-store (the full unfiltered graph, used for hover BFS).
    ...
    **Tree node interaction:**
    ...
    - **Hover:** On mouse enter:
      - For type nodes: `setHoveredCard(node.id, nodes, edges)` (where `nodes` and `edges` are the full unfiltered graph from viewer-store).
      - For relation/permission nodes: `setHoveredRow(node.id, edges)` (same `edges` reference).
